...
public class CandleStickChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    Transformer trans;
    private float[] mImg = new float[4];
        ...
        for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
            ...
            if (showCandleBar) {
                ...
                if (open > close) { // 下降
                    ...
                    //画矩形float left, float top, float right, float bottom
                    c.drawRect(
                    ...
                    //画矩形float left, float top, float right, float bottom
                    c.drawRect(
                            mBodyBuffers[0], mBodyBuffers[1], mBodyBuffers[2], mBodyBuffers[3],
                            mRenderPaint);
                } else { //相等的值
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
                        ...
                        switch (entry.getData().toString()) {
                            case "1":
                                //float left, float top, float right, float bottom
//                                mImg[0] = x - (icon.getIntrinsicWidth() / 2);
//                                mImg[1] = mBodyBuffers[1];
//                                mImg[2] = x + (icon.getIntrinsicWidth() / 2);
//                                mImg[3] = mBodyBuffers[3];
//                                trans.pointValuesToPixel(mImg);
...
//                                c.draw(bitmap, icon.getIntrinsicWidth(), icon.getIntrinsicHeight(), mRenderPaint);
                               ...
                               /* Rect mDrawableBoundsCache = new Rect();
                                ...
                                icon.copyBounds(mDrawableBoundsCache);
                                icon.setBounds(
                                        mDrawableBoundsCache.left,
                                        mDrawableBoundsCache.top,
                                        mDrawableBoundsCache.left + icon.getIntrinsicWidth(),
                                        mDrawableBoundsCache.top + icon.getIntrinsicWidth());
...
//                                        + "\n>>>>开盘>>" + mBodyBuffers[3] + ">>>>" + mShadowBuffers[3] + ">>" + entry.getOpen()
//                                        + "\n>>>>收盘>" + mBodyBuffers[1] + ">>>" + mShadowBuffers[1] + ">>>" + entry.getClose());
    ...
    private Bitmap getResizeBitmap(BitmapDrawable drawable, Context context) {
//      把drawable转化为bitmap
